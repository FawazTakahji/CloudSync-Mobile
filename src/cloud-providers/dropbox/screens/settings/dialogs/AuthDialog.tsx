            ...
            <Dialog visible={true}
                ...
                <Dialog.Content style={styles.container}>
                    <Text variant={"bodyLarge"}>
                        Please sign in to your Dropbox account and grant access to this app.
                    </Text>
                </Dialog.Content>
                <Dialog.Actions>
                    <Button mode={"text"}
    ...
    async function authenticate() {
        try {
            ...
            try {
                ...
                log.error("Dropbox authentication error:", e);
            ...
            if (result.type === "cancel" || result.type === "dismiss") {
                ...
                log.error("Dropbox authentication error:", result.error);
                props.showAuthErrorDialog();
                return;
            ...
            if (request && request.codeVerifier) {
                ...
                try {
                    ...
                    log.error("An error occurred while exchanging the dropbox code:", e);
                    props.showAuthErrorDialog();
                    return;
                }

                if (!tokenResponse.refreshToken) {
                    log.error("Dropbox refresh token is null.");
                    props.showAuthErrorDialog();
                    return;
                } else {
                    try {
                ...
                if (!request) {
                    log.error("Dropbox request is null.");
                    props.showAuthErrorDialog();
                } else {
                    log.error("Dropbox request code verifier is null.");
                    props.showAuthErrorDialog();
                }
            }
        } finally {
